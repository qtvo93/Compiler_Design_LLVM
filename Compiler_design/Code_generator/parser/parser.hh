...
// Class definition for While loop
class WhileStatement : public Statement
{    
  protected:
    std::shared_ptr<Condition> condi;
    std::vector<std::shared_ptr<Statement>> while_taken_block;
 

    std::unordered_map<std::string, ValueType::Type> while_taken_local_vars;


  public:

    WhileStatement(std::unique_ptr<Condition> &_condi,
                std::vector<std::shared_ptr<Statement>> &_while_taken_block,

                std::unordered_map<std::string, 
                                   ValueType::Type> &_while_taken_local_vars)
    {
        type = StatementType::WHILE_STATEMENT;

        condi = std::move(_condi);
        while_taken_block = std::move(_while_taken_block);

        while_taken_local_vars = _while_taken_local_vars;

    }

    WhileStatement(const WhileStatement &_while)
        : condi(std::move(_while.condi))
        , while_taken_block(std::move(_while.while_taken_block))

        , while_taken_local_vars(_while.while_taken_local_vars)

    {}

    auto getWhileCond() { return condi.get(); }
    auto &getWhileTakenBlock() { return while_taken_block; }

    auto getWhileTakenBlockVars() { return &while_taken_local_vars; }
