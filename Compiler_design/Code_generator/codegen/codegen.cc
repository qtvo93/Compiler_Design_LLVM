 ...
 * Student name(s): Quoc Thinh Vo , Cody Tran
 * Date: 12/08/2021
 */
...
void Codegen::whileGen(std::string& parent_func_name, Statement *_statement)
{
    ...
    auto &taken_block = while_s->getWhileTakenBlock(); 
	local_vars_ref.push_back(while_s->getWhileTakenBlockVars());
    local_vars_tracker.emplace_back();
	...
	auto cond = condGen(while_s->getWhileCond());       // Create code to evaluate condition
