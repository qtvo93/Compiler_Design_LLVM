 ...
 * 
 * Student names(s): Quoc Thinh Vo , Cody Tran
                                                     ...
                                                     parent_func_name)
{
	...
	//advanceTokens();
    //parse assignment statement
    auto start_assignment = parseAssnStatement();
	assert(cur_token.isTokenSemicolon());
    advanceTokens();
    
	
    //Relational expression:
    auto relational_expr = parseCondition();
	assert(cur_token.isTokenSemicolon());
    advanceTokens();
    

    //parse assignment statement
    auto end_assignment = parseAssnStatement();
    advanceTokens();
	...
	std::vector<std::shared_ptr<Statement>> block_code;
    std::unordered_map<std::string, ValueType::Type> block_code_local_vars;
	local_vars_tracker.push_back(&block_code_local_vars);
	...
	while (true) {
        ...
        parseStatement(parent_func_name, block_code);

        // Check if we just finished an if/for statement
        if (block_code.back()->isStatementFor()) {
            // This RBrace is from the statement,
            // should not terminate.
            assert(cur_token.isTokenRBrace());
        }
	...
	std::unique_ptr<Statement> for_statement = 
        std::make_unique<ForStatement>(start_assignment,relational_expr,end_assignment,
                                      block_code,
                                      block_code_local_vars);
                                                     ...
                                                     parent_func_name)
{
    ...
    std::vector<std::shared_ptr<Statement>> while_taken_block_codes;
    std::unordered_map<std::string,ValueType::Type> while_taken_block_local_vars;
    local_vars_tracker.push_back(&while_taken_block_local_vars);
    while (true) {
        advanceTokens();
        if (cur_token.isTokenRBrace())
            break;

        parseStatement(parent_func_name, while_taken_block_codes);

        // Check if we just finished an if/for statement
        if (taken_block_codes.back()->isStatementWhile()) {
            // This RBrace is from the statement,
            // should not terminate.
            assert(cur_token.isTokenRBrace());
        }
    ...
    std::unique_ptr<Statement> while_statement = 
        std::make_unique<WhileStatement>(condi, 
                                      while_taken_block_codes,
                                      while_taken_block_local_vars);
...
std::unique_ptr<Expression> Parser::parseFactor()
{
    ...
    if (cur_token.isTokenPlus() || cur_token.isTokenMinus()) {
		...
		//If cur_expr_type == INT, create (INT, '0') token
		//else create (FLOAT, '0.0') token
		Token new_token;
		...
		//left = LiteralExpression object constructed with the above token
		//Advance tokens
		left = std::make_unique<LiteralExpression>(new_token);
		advanceTokens();
		
		//If current token is of type INT or FLOAT:
			//right = LiteralExpression object constructed with the current token
			//Advance tokens
		std::unique_ptr<Expression> right;
		...
		//else
			//right = Recursive call to parse factor
		else {
			right = parseFactor();
		}	
		//left = ArithExpression object constructed with left, right, and expr_type
