...
std::unique_ptr<Statement> Parser::parseAssnStatement()
{
    ...
    if (isTokenTypeKeyword(cur_token)) {
		...
		//Advance tokens
				expr = std::make_unique<LiteralExpression>(new_token);
                                                     ...
                                                     parent_func_name)
{
	...
	while (true) {
        ...
        // Check if we just finished an if/for statement
        if (block.back()->isStatementIf() || block.back()->isStatementFor()) {
            // This RBrace is from the statement,
            // should not terminate.
            assert(cur_token.isTokenRBrace());
        }
                                                     ...
                                                     parent_func_name)
{
    ...
    while (true) {
        ...
        //auto end_assignment = parseAssnStatement();
        // Check if we just finished an if/for statement
        if (while_taken_block_codes.back()->isStatementIf() ||while_taken_block_codes.back()->isStatementWhile()) {
            // This RBrace is from the statement,
            // should not terminate.
            assert(cur_token.isTokenRBrace());
        }
...
void WhileStatement::printStatement()
{
    ...
    std::cout << "  [Condition]\n";
    condi->printStatement();
    std::cout << "  [Taken Block]\n";
    std::cout << "  {\n";
    for (auto &code : while_taken_block) 
        code->printStatement();
